...
module AgentHelper
  ...
  def agent_controllers(agent, delimiter = ', ')
    return unless agent.controllers.present?
  ...
  def agent_type_icon(agent, agents)
    ...
    if control_count.positive? && receiver_count.positive?
      content_tag('span') do
        concat icon_tag('glyphicon-arrow-right')
        concat tag('br')
        concat icon_tag('glyphicon-new-window', class: 'glyphicon-flipped')
      end
    elsif control_count.positive? && receiver_count.zero?
      icon_tag('glyphicon-new-window', class: 'glyphicon-flipped')
    elsif receiver_count.positive? && source_count.zero?
      icon_tag('glyphicon-arrow-right')
    elsif receiver_count.zero? && source_count.positive?
      icon_tag('glyphicon-arrow-left')
    elsif receiver_count.positive? && source_count.positive?
  ...
  def agent_type_select_options
    Rails.cache.fetch('agent_type_select_options') do
      types = Agent.types.map do |type|
        [agent_type_to_human(type.name), type,
         { title: h(Agent.build_for_type(type.name, User.new(id: 0), {}).html_description.lines.first.strip) }]
      end
      types.sort_by! { |t| t[0] }
  ...
  def links_counter_cache(agents)
    ...
    @counter_cache[agents.__id__] ||= {}.tap do |cache|
      ...
      cache[:links_as_receiver] = Link.where(receiver_id: agent_ids)
                                           .group(:receiver_id)
                                           .pluck(:receiver_id, Arel.sql('count(receiver_id) as id')).to_h
      cache[:links_as_source]   = Link.where(source_id: agent_ids)
                                           .group(:source_id)
                                           .pluck(:source_id, Arel.sql('count(source_id) as id')).to_h
      cache[:control_links_as_controller] = ControlLink.where(controller_id: agent_ids)
                                                            .group(:controller_id)
                                                            .pluck(:controller_id, Arel.sql('count(controller_id) as id')).to_h
    end
  end
