  ...
  class DataOutputAgent < Agent
    ...
    description do
      <<~MD
        ...
        `https://#{ENV.fetch('DOMAIN', nil)}#{Rails.application.routes.url_helpers.web_requests_path(agent_id: ':id', user_id:, secret: ':secret', format: :xml)}`
    ...
    def validate_options
      if options['secrets'].is_a?(Array) && options['secrets'].length.positive?
    ...
    def feed_link
      interpolated['template']['link'].presence || "https://#{ENV.fetch('DOMAIN', nil)}"
    end
    ...
    def itunes_icon
      "<itunes:image href=#{feed_icon.encode(xml: :attr)} />" if boolify(interpolated['ns_itunes'])
    end
    ...
    def xml_namespace
      ...
      namespaces << 'xmlns:dc="http://purl.org/dc/elements/1.1/"' if boolify(interpolated['ns_dc'])
      namespaces << 'xmlns:media="http://search.yahoo.com/mrss/"' if boolify(interpolated['ns_media'])
      namespaces << 'xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd"' if boolify(interpolated['ns_itunes'])
      namespaces.join(' ')
    end
    ...
    def latest_events(reload = false)
      ...
      memory[:event_ids] = events.map(&:id) if reload
    ...
    def receive_web_request(params, _method, format)
      ...
      interpolate_with('events' => source_events) do
        ...
        if format.include?('json')
          ...
          hub_links = push_hubs.map do |hub|
            <<-XML
    ...
    def items_to_xml(items)
      simplify_item_for_xml(items)
        ...
        .gsub(%r{
          ...
          \k<indent> </ \k<tagname> > \n
        }mx) { $LAST_MATCH_INFO[:children].gsub(/^ /, '') } # delete redundant nesting of array elements
        .gsub(%r{
          (?<indent> ^\ + ) < [^> ]+ /> \n
