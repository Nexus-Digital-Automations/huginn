...
class ErrorMonitoringController < ApplicationController
  ...
  def generate_alert_summary
    ...
    open_circuits = cb_status[:services].select { |_name, status| status[:state] == :open }
  ...
  def fetch_severity_trends(time_range)
    ...
    AgentLog.where('level >= ? AND created_at > ?', 3, time_range.ago)
            ...
            .group(:level)
            .count
            .group_by { |(hour, _level), _count| hour }
            .transform_values do |hour_data|
              severity_counts = {}
              hour_data.each { |(_hour, level), count| severity_counts[level_to_severity(level)] = count }
              severity_counts
            end
  rescue StandardError => e
  ...
  def fetch_strategy_performance(_time_range)
  ...
  def calculate_performance_metrics(time_range)
...

    {
      ...
      error_rate: total_requests.positive? ? (total_errors.to_f / total_requests) : 0,
  ...
  def handle_controller_error(_error, user_message)
    respond_to do |format|
